...
def list_appointments(
    ...
    q: str | None = Query(default=None, description="Search in note or patient/doctor name"),
...
def assign_appointment(
    ...
    db: Session = Depends(get_db),
    user: User = Depends(require_roles(UserRole.doctor, UserRole.admin, UserRole.registrar)),
):
    doctor_id = payload.doctor_id
    if user.role == UserRole.doctor and user.doctor:
        doctor_id = user.doctor.id
...
def update_appointment(
    ...
    if payload.status == AppointmentStatus.cancelled:
        ...
        return enrich_appointment(appointment)
    if payload.status:
        if user.role not in (UserRole.admin, UserRole.doctor, UserRole.registrar):
...
def reschedule(
    ...
    appointment = reschedule_appointment(
        db, appointment, user, payload.starts_at, payload.note
    )
    return enrich_appointment(appointment)
